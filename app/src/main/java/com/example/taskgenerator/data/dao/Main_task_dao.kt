...
import com.example.taskgenerator.data.entity.MainTaskWithSubTasks
...
interface Main_task_dao {
    ...
    suspend fun insertMainTask(task: Main_task_entity)

    @Update
    suspend fun updateMainTask(task: Main_task_entity)

    @Delete
    suspend fun deleteMainTask(task: Main_task_entity)
    ...
    @Transaction
    @Query("SELECT * FROM main_task ORDER BY createdAt DESC")
    fun getAllMainTasksWithSubTasks(): Flow<List<MainTaskWithSubTasks>>

    @Transaction
    @Query("SELECT * FROM main_task WHERE mainTaskId = :id")
    fun getMainTaskWithSubTasks(id: Long): Flow<MainTaskWithSubTasks?>
}